...
const googleMap = () => {
    const MAP_TOKEN = 'AIzaSyDgxaAVu6wZkfdefa5F1tDC6bVGXvLTqg0';
    ...
    return (<>
        <LoadScript
            googleMapsApiKey={MAP_TOKEN}
            region={"kr"}
        >
            <GoogleMap
                mapContainerStyle={{
                    width: '100%', // 지도 가로 길이
                    height: '850px' // 지도 세로 길이
                }}
                // center={{ lat: 37.553818, lng: 126.886020 }}
                center={currentPosition}
                zoom={14}
            >
                {
                    currentPosition.lat ?
                        <Marker class={"crosshair"}
                                position={currentPosition}
                                icon={"https://image.flaticon.com/icons/png/128/487/487021.png"}
                        />
                        :null
                }
                {
