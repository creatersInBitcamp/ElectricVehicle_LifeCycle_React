                            ...
                            <div className="dashboard-right">
                                <NaverMap
                                    ...
                                    style={{
                                        ...
                                        height: '850px' // 네이버지도 세로 길이
                                    }}
                                    defaultCenter={{ lat: 37.546223, lng: 126.898838 }} // 지도 초기 위치
                                ...
                                >
                                    <SideBar/>
                                    {
                                        storeList.map((store, i) => (
                                            <Marker
                                                key={i}
                                                position={new navermaps.LatLng(store.location.lat, store.location.lng)}
                                                animation={1} //마커 애니메이션을 설정할 수 있다. 애니메이션은 0, 1, 2 등 3가지 종류가 있다.
                                                onClick={() => {alert(`여기는 ${store.name}입니다.`);}}
                                                title={store.name}
                                            >
                                            </Marker>
                                        ))
...
const MapNaver = () => {
    return (
        <RenderAfterNavermapsLoaded
            ncpClientId={'ghb6qgihki'} // 자신의 네이버 계정에서 발급받은 Client ID
