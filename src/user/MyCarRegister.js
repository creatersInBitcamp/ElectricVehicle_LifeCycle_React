...
const addToMyCar = ({product,user}) => {
    ...
    axios.post(`http://${BACK_PATH}/usedCars/register`, info)
        .then((res)=>{
...
const removeAllCar = (myCars) => (dispatch) => {
    axios.post(`http://${BACK_PATH}/usedCars/deleteAllCar`,myCars)
...
const changeFirstCar = (myCars,targetId) => {
    ...
    axios.get(`http://${BACK_PATH}/usedCars/updateFirstCar/${product.usedCarId}`)
        .then((res)=>{
            window.location.reload()
        })
        .catch((err)=>{ throw err })
    ...
    if (before) {
        axios.post(`http://${BACK_PATH}/usedCars/updateFirstCar`,before)
            .then((res)=>{})
            .catch((err)=>{ throw err })
    }
}
...
export const myCarReducer = (state=initialState, action) => {
    switch (action.type) {
        ...
        case REMOVE_FROM_MY_CAR:
            axios.get(`http://${BACK_PATH}/usedCars/deleteMyCar/${action.product_id}`)
        ...
        case REMOVE_ALL_FIRST_CAR:
            ...
            axios.get(`http://${BACK_PATH}/usedCars/deleteMyFirstCar/${action.firstCar[0].usedCarId}`)
                .then((res)=> res.data, window.location.reload())
                .catch(()=> alert(`삭제 실패`))
            return {
                list:[]
...
export const MyCarRegister = () => {
    ...
    useEffect(()=> {
        ...
        if (used.length > 0){
            ...
            axios.get(`http://${BACK_PATH}/usedCars/getDetailList/${userSession.userSeq}`)
                .then((res)=>{
                    setProduct(res.data)
                })
        ...
        axios.get(`http://${BACK_PATH}/usedCars/getAllMyCar/${userSession.userSeq}`)
            .then((res)=>{
